...
@Component({
  selector: 'app-evaluada',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './evaluada.component.html',
  styleUrl: './evaluada.component.css',
})
export class EvaluadaComponent implements OnInit, OnDestroy {
  ordenes: any[] = [];
  cargando = false;
  ...
  ordenObservacion: any | null = null;

  
  descargando: { id: number; formato: string } | null = null;
  ...
  ngOnDestroy(): void { this._querySub?.unsubscribe(); }

  cargarOrdenes(): void {
    this.cargando = true;
    this.procesoSrv.ListarOrdenes('evaluada').subscribe({
      next: (resp: any) => {
        ...
        if (this._autoOpenReporte) {
          ...
          if (orden) setTimeout(() => this.verReporteTecnico(orden), 0);
        }
  ...
  verObservacion(o: any): void { this.ordenObservacion = o; }
  cerrarObservacion(): void    { this.ordenObservacion = null; }

  descargarPDF(o: any): void {
    this.descargando = { id: o.idorden, formato: 'pdf' };
    this.cdr.detectChanges();

    this.procesoSrv.DescargarPDFReporte(o.idorden).subscribe({
      next: (blob: Blob) => {
        const url = URL.createObjectURL(new Blob([blob], { type: 'application/pdf' }));
        const a = document.createElement('a');
        a.href = url;
        a.download = `reporte_orden_${o.idorden}.pdf`;
        a.click();
        URL.revokeObjectURL(url);
        this.descargando = null;
        this.cdr.detectChanges();
      },
      error: () => { this.descargando = null; this.cdr.detectChanges(); },
    });
  }

  descargarFotos(o: any): void {
    this.descargando = { id: o.idorden, formato: 'fotos' };
    this.cdr.detectChanges();

    this.procesoSrv.DescargarFotosZIP(o.idorden).subscribe({
      next: (blob: Blob) => {
        const url = URL.createObjectURL(new Blob([blob], { type: 'application/zip' }));
        const a = document.createElement('a');
        a.href = url;
        a.download = `fotos_orden_${o.idorden}.zip`;
        a.click();
        URL.revokeObjectURL(url);
        this.descargando = null;
        this.cdr.detectChanges();
