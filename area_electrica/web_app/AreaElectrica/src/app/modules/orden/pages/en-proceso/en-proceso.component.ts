...
@Component({
  ...
  styleUrl: './en-proceso.component.css',
})
export class EnProcesoComponent implements OnInit, OnDestroy {
  ordenes: any[] = [];
  cargando = false;
  ...
  cargarOrdenes(): void {
    this.cargando = true;
    this.procesoSrv.ListarOrdenes('proceso').subscribe({
      next: (resp: any) => { this.ordenes = resp.datos ?? []; this.currentPage = 1; this.cargando = false; this.cdr.detectChanges(); },
